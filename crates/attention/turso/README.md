...
- One process exclusively owns one absolute, normalized, owner-only dedicated database directory. Database and backup roots are created and validated by walking from an opened root descriptor with descriptor-relative, no-follow operations; ownership locks are opened and validated relative to the retained database-directory descriptor. Public path accessors remain normalized display and legacy-consumer paths, not evidence of current filesystem identity.
