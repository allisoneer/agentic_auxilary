...
use crate::model::{RouterDef, ToolDef, ParamSource, HttpMethod};
...
pub fn generate_rest_methods_split(router: &RouterDef) -> (TokenStream, TokenStream) {
    ...
    let module_name = syn::Ident::new(
        &format!("__utf_rest_generated_{}", struct_type.segments.last().unwrap().ident.to_string().to_lowercase()),
        struct_type.span()
    );
...
fn generate_create_rest_router(router: &RouterDef, module_name: &syn::Ident) -> TokenStream {
    let routes = router.tools.iter().map(|tool| {
        generate_tool_route(tool, module_name)
    });
    ...
    let _prefix = router.metadata.base_path.as_deref()
        .unwrap_or("/api");
...
fn generate_handler(tool: &ToolDef, module_name: &syn::Ident) -> TokenStream {
    let has_body_params = tool.params.iter().any(|p| matches!(p.source, ParamSource::Body));
    ...
    let method_args_vec: Vec<TokenStream> = tool.params.iter().map(|param| {
        let name = &param.name;
        match param.source {
...
fn generate_openapi_schemas(router: &RouterDef) -> TokenStream {
    let schemas = router.tools.iter()
        .filter(|tool| tool.params.iter().any(|p| matches!(p.source, ParamSource::Body)))
        .map(|_tool| {
            quote! {
