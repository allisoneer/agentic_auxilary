...
use quote::quote;
use crate::model::{RouterDef, ToolDef, ParamSource};
use crate::codegen::validation;
use crate::codegen::shared::{type_to_string, to_kebab_case};
...
fn generate_create_cli_command(router: &RouterDef) -> TokenStream {
    ...
    let cli_name = router.metadata.cli_config.as_ref()
        .and_then(|c| c.name.as_ref())
        .cloned()
        .unwrap_or_else(|| {
            router.struct_type.segments.last()
                .map(|s| s.ident.to_string().to_lowercase())
                .unwrap_or_else(|| "tools".to_string())
        });
    
    let about = router.metadata.cli_config.as_ref()
    ...
    let subcommands: Vec<_> = router.tools.iter().map(|tool| {
        generate_tool_subcommand(tool)
    }).collect();
    ...
    let has_global_args = router.metadata.cli_config.as_ref()
        .map(|c| !c.global_output_formats.is_empty())
        .unwrap_or(false);
    
    let has_standard_args = router.metadata.cli_config.as_ref()
...
fn generate_tool_subcommand(tool: &ToolDef) -> TokenStream {
    ...
    let cli_name_str = tool.metadata.cli_config.as_ref()
        .and_then(|c| c.name.as_ref())
    ...
    let args: Vec<_> = tool.params.iter()
    ...
    let alias_calls = if let Some(cli_config) = &tool.metadata.cli_config {
        cli_config.aliases.iter().map(|alias| {
            quote! { .alias(#alias) }
        }).collect::<Vec<_>>()
    } else {
        vec![]
    ...
    if std::env::var("UTF_DEBUG").is_ok() {
        eprintln!("Building subcommand for tool: {} with {} args", cli_name_str, args.len());
    }
...
fn generate_execute_cli(router: &RouterDef) -> TokenStream {
    let match_arms: Vec<_> = router.tools.iter().map(|tool| {
        generate_execute_match_arm(tool, router)
    }).collect();
...
fn generate_execute_match_arm(tool: &ToolDef, router: &RouterDef) -> TokenStream {
    ...
    let param_names: Vec<_> = tool.params.iter()
        .map(|p| &p.name)
        .collect();
    
    // Check if tool has output formats configured
    let has_output_formats = tool.metadata.cli_config.as_ref()
        .map(|c| !c.output_formats.is_empty())
        .unwrap_or(false);
    
    // Check if global output formats are configured
    let _output_handling = if has_output_formats && router.metadata.cli_config.as_ref()
        .map(|c| !c.global_output_formats.is_empty()).unwrap_or(false) {
        quote! {
    ...
    let param_args_vec: Vec<TokenStream> = param_names.iter()
        .map(|name| quote! { #name })
        .collect();
    ...
    let _has_confirm = tool.metadata.cli_config.as_ref()
