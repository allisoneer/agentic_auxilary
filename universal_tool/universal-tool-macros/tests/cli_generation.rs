...
use universal_tool_macros::{universal_tool_router, universal_tool};
...
#[universal_tool_router(
    cli(name = "test-cli", description = "Test CLI for integration tests")
)]
impl TestTools {
    /// Basic add function
    #[universal_tool(
    ...
    #[universal_tool(
        description = "Format with prefix",
        cli(name = "format")
    )]
    async fn format_text(
        &self,
        text: String,
        #[universal_tool_param(env = "TEST_PREFIX", default = ">>>")]
    ...
    async fn concat(
        ...
        a: String,
        b: String,
        #[universal_tool_param(default = " ", description = "Separator")]
        sep: Option<String>
    ) -> Result<String, ToolError> {
...
mod tests {
    ...
    fn test_cli_command_generation() {
        ...
        assert_eq!(cmd.get_about().map(|s| s.to_string()), Some("Test CLI for integration tests".to_string()));
    ...
    fn test_aliases() {
        ...
        let is_even_cmd = cmd.get_subcommands()
            .find(|c| c.get_name() == "is-even")
            .expect("is-even command not found");
        
        // Check aliases
        let aliases: Vec<_> = is_even_cmd.get_all_aliases()
            .map(|a| a.to_string())
            .collect();
    ...
    fn test_hidden_command() {
        ...
        let debug_cmd = cmd.get_subcommands()
    ...
    fn test_default_values() {
        ...
        let concat_cmd = cmd.get_subcommands()
            .find(|c| c.get_name() == "concat")
            .expect("concat command not found");
        
        // Find the separator argument
        let sep_arg = concat_cmd.get_arguments()
            .find(|a| a.get_id() == "sep")
            .expect("sep argument not found");
        
        // Check default value
        let default_values: Vec<_> = sep_arg.get_default_values()
            .into_iter()
    ...
    async fn test_execute_cli() {
        ...
        let matches = app.clone().try_get_matches_from(vec!["test-cli", "add", "--a", "5", "--b", "3"])
