...
impl FileManager {
    ...
    fn log_operation(&self, operation: String) {
        if let Ok(mut log) = self.operation_log.lock() {
            log.push(format!("{}: {}", Utc::now().format("%Y-%m-%d %H:%M:%S"), operation));
        }
    }
...
impl FileManager {
    ...
    async fn list_files(
        ...
        #[universal_tool_param(description = "Include hidden files", source = "query")] include_hidden: Option<bool>,
        #[universal_tool_param(description = "Recursive listing", source = "query")] recursive: Option<bool>,
        ...
        self.log_operation(format!("Listed directory: {} (found {} files, {} dirs)", path, file_count, dir_count));
    ...
    async fn create_file(
        ...
        if resolved_path.exists() && !overwrite {
            return Err(ToolError::conflict(format!("File already exists: {}", request.path)));
        }
        ...
        self.log_operation(format!("Created file: {} ({} bytes)", request.path, request.content.len()));
    ...
    async fn delete_file(
        ...
        #[universal_tool_param(description = "Recursive deletion for directories", source = "query")] recursive: Option<bool>,
    ) -> Result<(), ToolError> {
        let resolved_path = self.resolve_path(&path);
    ...
    async fn copy_file(
        ...
        if !source_path.exists() {
            return Err(ToolError::not_found(format!("Source not found: {}", request.source)));
        }
        
        if dest_path.exists() && !overwrite {
            return Err(ToolError::conflict(format!("Destination already exists: {}", request.destination)));
        }
        ...
        if source_path.is_file() {
            ...
            return Err(ToolError::invalid_input("Directory copying not implemented".to_string()));
        }

        let metadata = fs::metadata(&dest_path)
            .map_err(|e| ToolError::internal(format!("Failed to get metadata: {}", e)))?;

        self.log_operation(format!("Copied {} to {}", request.source, request.destination));
    ...
    async fn search_files(
        ...
        #[universal_tool_param(description = "Directory to search in", source = "query")] path: String,
        #[universal_tool_param(description = "File extensions to search", source = "query")] extensions: Option<String>,
        #[universal_tool_param(description = "Case sensitive search", source = "query")] case_sensitive: Option<bool>,
        ...
        self.log_operation(format!("Searched for '{}' in {} ({} files, {} matches)", 
    ...
    async fn get_stats(
        ...
        for entry in WalkDir::new(&resolved_path) {
            ...
            if metadata.is_dir() {
                ...
                if largest_file.as_ref().map_or(true, |f| file_info.size > f.size) {
                    largest_file = Some(file_info.clone());
                }

                if most_recent_file.as_ref().map_or(true, |f| file_info.modified > f.modified) {
...
#[derive(Parser)]
#[command(author, version, about = "File Manager - CLI, REST API, and MCP server")]
struct Args {
            ...
            Ok(request) => {
                ...
                let response = match method {
                    ...
                    "tools/call" => {
                        ...
                        match file_manager.handle_mcp_call(tool_name, tool_params.clone()).await {
                            Ok(result) => {
                                serde_json::json!({
                                    "jsonrpc": "2.0",
                                    "id": id,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.rest {
        ...
        info!("API endpoints available at: http://localhost:{}/api/v1", args.port);
        ...
        let matches = file_manager.create_cli_command()
