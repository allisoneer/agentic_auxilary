...
impl ServerHandler for RegistryServer {
    ...
    ) -> impl std::future::Future<Output = Result<m::CallToolResult, m::ErrorData>> + Send + '_
    {
        async move {
            ...
            match self.registry.dispatch_json(&req.name, args, &ctx).await {
                Ok(out) => {
                    let content = match self.output_mode {
                        OutputMode::Json => m::Content::json(out)
                            .unwrap_or_else(|_| m::Content::text("json serialization error")),
                        OutputMode::Text => {
                    ...
                    Ok(m::CallToolResult::success(vec![content]))
                }
