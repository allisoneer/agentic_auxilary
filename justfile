...
# review-prepare: Write ./review.diff + ./review.meta.json. mode=default|staged; paths=whitespace-separated pathspecs
review-prepare mode="default" paths="":
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    case "$MODE" in
      default|staged) ;;
      *) echo "Invalid mode: '$MODE'. Allowed: default|staged" >&2; exit 2 ;;
    esac
    ...
    diff_path="$repo_root/review.diff"
    meta_path="$repo_root/review.meta.json"
    ...
    # Parse optional pathspecs (space-separated)
    pathspec=()
    if [ -n "$PATHS_STR" ]; then
    ...
    if [ "$MODE" = "staged" ]; then
      ...
      base_ref_name="HEAD"
      head_ref="INDEX"
      diff_cmd=(git --no-pager diff --cached --no-color --no-ext-diff)
      name_cmd=(git --no-pager diff --cached --name-only)
      shortstat_cmd=(git --no-pager diff --cached --shortstat)
    else
      base_ref_name="origin/main"
      if git rev-parse --verify -q "$base_ref_name" >/dev/null; then
        base_ref="$(git merge-base HEAD "$base_ref_name")"
      else
        for ref in origin/master main master; do
      ...
      if [ -z "$base_ref" ]; then
        echo "Unable to compute merge-base with origin/main (or fallbacks). Try: git fetch --all" >&2
        exit 1
      fi

      head_ref="WORKTREE"
      diff_cmd=(git --no-pager diff --no-color --no-ext-diff "$base_ref")
      name_cmd=(git --no-pager diff --name-only "$base_ref")
      shortstat_cmd=(git --no-pager diff --shortstat "$base_ref")
    fi

    # Write diff + capture metadata inputs
    if [ "${#pathspec[@]}" -gt 0 ]; then
      "${diff_cmd[@]}" -- "${pathspec[@]}" > "$diff_path" || true
      changed_files="$("${name_cmd[@]}" -- "${pathspec[@]}" || true)"
      shortstat="$("${shortstat_cmd[@]}" -- "${pathspec[@]}" || true)"
    else
      "${diff_cmd[@]}" > "$diff_path" || true
      changed_files="$("${name_cmd[@]}" || true)"
      shortstat="$("${shortstat_cmd[@]}" || true)"
    fi

    diff_bytes="$(wc -c < "$diff_path" | tr -d ' ')"
    diff_lines="$(wc -l < "$diff_path" | tr -d ' ')"
    has_changes="false"
    if [ "${diff_bytes:-0}" -gt 0 ]; then
      has_changes="true"
    fi
    ...
    # Convert changed_files to JSON array
    changed_files_json="[]"
