use crate::config::{MountDirs, MountPattern, PersonalConfig, PersonalMount, Rule};
...
mod tests {
    ...
    fn test_save_and_load_personal_config() {
        with_temp_home(|| {
            ...
            assert_eq!(
                loaded.patterns[0].match_remote,
                config.patterns[0].match_remote
            );
        });
    }
    ...
    fn test_repository_mounts() {
        with_temp_home(|| {
            ...
            // Remove mount
            assert!(
                PersonalConfigManager::remove_repository_mount(repo_url, &mount.mount_path)
                    .unwrap()
            );
            ...
            assert!(
                !PersonalConfigManager::remove_repository_mount(repo_url, "nonexistent").unwrap()
            );
        });
    }
