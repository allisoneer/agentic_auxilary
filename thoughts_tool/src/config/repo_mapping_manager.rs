use super::types::{RepoMapping, RepoLocation};
...
impl RepoMappingManager {
    ...
    pub fn load(&self) -> Result<RepoMapping> {
        ...
        let mapping: RepoMapping = serde_json::from_str(&contents)
            .context("Failed to parse repository mapping")?;
        Ok(mapping)
    }
...
mod tests {
    ...
    fn test_extract_repo_name() {
        assert_eq!(extract_repo_name_from_url("git@github.com:user/repo.git").unwrap(), "repo");
        assert_eq!(extract_repo_name_from_url("https://github.com/user/repo").unwrap(), "repo");
        assert_eq!(extract_repo_name_from_url("git@github.com:user/repo").unwrap(), "repo");
    }
}