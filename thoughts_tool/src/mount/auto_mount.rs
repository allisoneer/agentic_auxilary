use crate::mount::{MountManager, get_mount_manager, MountOptions};
use crate::config::{MountMerger, Mount, MountSource, RepoConfigManager};
use crate::git::clone::{clone_repository, CloneOptions};
...
pub async fn update_active_mounts() -> Result<()> {
    ...
    for mount_info in &active_mounts {
        ...
        if mount_info.target.starts_with(repo_root.join(".thoughts-data")) {
            if let Some(name) = mount_info.target.file_name()
                .and_then(|n| n.to_str()) {
    ...
    for (active_name, active_path) in &active_map {
        if !desired_mounts.contains_key(active_name) {
            ...
            mount_manager.unmount(&active_path, false).await
    ...
    for (name, (mount, source)) in &desired_mounts {
        if !active_map.contains_key(name) {
            ...
            if !mount_path.exists() {
                eprintln!("    {} Mount path does not exist: {}", "Error:".red(), mount_path.display());
                continue;
            }
